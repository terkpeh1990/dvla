...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if order.gross_price > form.cleaned_data['money']:
                messages.success(request,"Amount Entered Cannot be less  than total transaction")
                ...
                Revenue.objects.create(
                    account_code=code, amount=order.money_paid,hq=hq['cc'],office=office['cc'],trip = order.trip)
...
def income_expenditure(request):
    ...
    hq = Revenue.objects.aggregate(cc=Sum('hq'))
    office =Revenue.objects.aggregate(cc=Sum('office'))
...
def makepayment(request,pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Revenue.objects.create(
                account_code=code, amount=dd, hq=tot['hq'], office=tot['igf'])
